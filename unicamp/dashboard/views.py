...
class HomeView(View):  # definimos a classe HomeView que será chamada dentro do urls.py
    def get(self, request, *args,
        ...
        carga = df.groupby(['UNIDADE', 'PAPEL']).count()['USUARIO_ID'].reset_index()

        # Tratamento dos dados. Remoção de duplicatas de usuários de cada uma das instituições e posterior combinação em um único dataframe
        pessoas_U = df[df['INSTITUICAO'] == 'UNICAMP'].drop_duplicates('USUARIO_ID')
        pessoas_C = df[df['INSTITUICAO'] == 'COTUCA'].drop_duplicates('USUARIO_ID')
        pessoas_L = df[df['INSTITUICAO'] == 'COTIL'].drop_duplicates('USUARIO_ID')
        pessoas_P = df[df['PAPEL'] == 'PAD/PED'].drop_duplicates('USUARIO_ID')
        frames = [pessoas_U, pessoas_C, pessoas_L, pessoas_P]
        ...
        disc_U = df[df['INSTITUICAO'] == 'UNICAMP'].drop_duplicates('nome_longo')
        disc_C = df[df['INSTITUICAO'] == 'COTUCA'].drop_duplicates('nome_longo')
        disc_L = df[df['INSTITUICAO'] == 'COTIL'].drop_duplicates('nome_longo')
        disc_P = df[df['PAPEL'] == 'PAD/PED'].drop_duplicates('nome_longo')
        frames = [disc_U, disc_C, disc_L, disc_P]
        disc = pd.concat(frames)

        # Organização da estrutura de dados, como contagem do número de pessoas por instituição e por papel
        bar_df = pessoas.groupby(['INSTITUICAO', 'PAPEL']).count()['USUARIO_ID'].reset_index()
        bar_df.rename(columns={'USUARIO_ID': 'Contagem'}, inplace=True)

        # Organização da estrutura de dados, como contagem do número de pessoas por instituição e por papel
        bar_df_disc = disc.groupby(['INSTITUICAO', 'NIVEL']).count()['nome_longo'].reset_index()
        bar_df_disc.rename(columns={'nome_longo': 'Contagem'}, inplace=True)

        # Dados para o gráfico de pizza
        pie_df_disc = disc.groupby(['UNIDADE']).count()['nome_longo'].reset_index()
        pie_df_disc.rename(columns={'nome_longo': 'Contagem'}, inplace=True)

        # Dados para gráfico de barra carga por unidade
        carga2 = carga.groupby(['UNIDADE', 'PAPEL']).mean()['USUARIO_ID'].reset_index()
        carga2.rename(columns={'USUARIO_ID': 'Contagem'}, inplace=True)

        ###################################################################
        # Gráfico de barras: Número de alunos e professores por instituição
        bar = px.bar(bar_df, x="INSTITUICAO", y="Contagem", color="PAPEL",
                     title="Número de alunos e professores por instituição",
                     labels={
                         "INSTITUICAO": "Instituição",
                         "Contagem": "Número de alunos/professores",
                         "PAPEL": "Função"
                     }, color_discrete_map={'PROFESSOR': '#2A2F33', 'ALUNO': '#8C9493', 'PAD/PED': '#8B8B94'},
                     custom_data=["PAPEL"])
        ...
        bar2 = px.bar(bar_df_disc, x="INSTITUICAO", y="Contagem", color="NIVEL",
                      ...
                      color_discrete_map={'Ensino Médio': '#73909a', 'Graduação': '#ebd2b5', 'Pós Graduação': '#99621f'}
                      )

        pie = px.pie(pie_df_disc, values='Contagem', names='UNIDADE', title='Proporção de disciplinas por Unidade',
                     labels={
                         'Contagem': 'Número de disciplinas',
                         "UNIDADE": 'Unidade'
                     },
        ...
        pie2 = px.pie(bar_df_disc, values='Contagem', names='INSTITUICAO',
                      title='Proporção de disciplinas por Instituição',
                      labels={
                          'Contagem': 'Número de disciplinas',
                          "INSTITUICAO": 'Instituição'
                      },
        ...
        pie3 = px.pie(bar_df_disc, values='Contagem', names='NIVEL', title='Proporção de disciplinas por nível',
                      labels={
                          'Contagem': 'Número de disciplinas',
                          "NIVEL": 'Nível'
                      },
        ...
        bar3 = px.bar(carga2, x="UNIDADE", y="Contagem", color="PAPEL",
                      title="Número de alunos e professores por Unidade",
                      labels={
                          "UNIDADE": "Unidade",
                          "Contagem": "Número de alunos/professores por Unidade",
                          "PAPEL": "Papel"
                      },
                      color_discrete_sequence=px.colors.qualitative.Antique,
        ...
        fig3 = plot(pie, output_type='div')
        fig4 = plot(pie2, output_type='div')
        fig5 = plot(pie3, output_type='div')
        fig6 = plot(bar3, output_type='div')
        ctx = {'fig1': fig1, 'fig2': fig2, 'fig3': fig3, 'fig4': fig4, 'fig5': fig5, 'fig6': fig6}
